...
describe RandomGenerator do
  ...
  it 'generates a random number between 6 - 10' do
    allow(subject.numbers).to receive(:sample) { 6 }
    expect(subject.random_number).to eq 6
  end

  it 'generates a random between 1 - 10' do
    allow(subject.all_numbers).to receive(:sample) { 1 }
    expect(subject.random_all_number).to eq 1
  end
end
